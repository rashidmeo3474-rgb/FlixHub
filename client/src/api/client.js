...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    // Handle network/database connection issues with mock data
    if (error.code === 'ECONNREFUSED' || error.response?.status >= 500) {
      const path = error.config?.url || '';
      
      // Mock response for authentication with proper credential validation
      if (path.includes('/auth/login') || path.includes('/auth/admin/login')) {
        const { email, password } = error.config?.data ? JSON.parse(error.config.data) : {};
        
        // Mock admin login - validate credentials
        if (path.includes('/admin/login')) {
          if ((email === 'admin@flixhub.pk' || email === 'businessyttom@gmail.com') && password === 'admin123') {
            ...
            // Return proper login error for wrong credentials
        ...
        // Mock regular user login with validation  
        if (email && password && email.includes('@')) {
          ...
          // Return error for missing or invalid credentials
          return Promise.reject({
            response: {
              status: 401,
              data: { message: 'Wrong email or password' }
      ...
      // Mock response for regular inventory
      if (path.includes('/inventory') && !path.includes('/summary')) {
        return Promise.resolve({
          data: {
            inventory: [
              { product: { _id: '1', name: 'Netflix', accent: '#e50914' }, totalAccounts: 3, totalSlots: 12, available: 8, occupied: 4 },
      ...
      // Mock response for admin accounts and subscriptions
      if (path.includes('/admin/accounts') || path.includes('/subscriptions')) {
        return Promise.resolve({
          data: {
            accounts: [
              {
      ...
      if (path.includes('/products')) {
        return Promise.resolve({
          data: {
            products: [
              { _id: '1a', name: 'Netflix Mobile', slug: 'netflix-480p', quality: '480p SD', monthlyPrice: 250, compareAt: 350, accent: '#e50914', category: 'movies', inStock: 12, active: true, logo: '/logos/netflix.jpg' },
              { _id: '1b', name: 'Netflix Basic', slug: 'netflix-720p', quality: '720p HD', monthlyPrice: 350, compareAt: 450, accent: '#e50914', category: 'movies', inStock: 10, active: true, logo: '/logos/netflix.jpg' },
              ...
              { _id: '2', name: 'Netflix + Prime Video', slug: 'netflix-prime', quality: '4K UHD', monthlyPrice: 600, compareAt: 1000, accent: '#ff6b00', category: 'bundle', inStock: 5, active: true, logo: '/logos/netflix-prime-shop.png' },
              { _id: '3', name: 'Prime Video', slug: 'prime-video', quality: '4K UHD', monthlyPrice: 350, compareAt: 500, accent: '#00a8e1', category: 'movies', inStock: 8, active: true, logo: '/uploads/images (5).jpeg' },
              { _id: '4a', name: 'HBO Max Basic', slug: 'hbo-max-480p', quality: '480p SD', monthlyPrice: 300, compareAt: 450, accent: '#9b30ff', category: 'movies', inStock: 9, active: true, logo: '/logos/hbo-max-shop.png' },
              { _id: '4', name: 'HBO Max Premium', slug: 'hbo-max', quality: '4K UHD', monthlyPrice: 450, compareAt: 700, accent: '#9b30ff', category: 'movies', inStock: 6, active: true, logo: '/logos/hbo-max-shop.png' },
              { _id: '5', name: 'Disney+', slug: 'disney', quality: '4K UHD', monthlyPrice: 400, compareAt: 550, accent: '#4b6cf7', category: 'movies', inStock: 8, active: true, logo: '/uploads/images (3).jpeg' },
