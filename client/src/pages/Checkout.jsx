...
const METHODS = [
  ...
  { key: 'nayapay', label: 'NayaPay', note: 'Mobile wallet · instant approval' },
  { key: 'ubl', label: 'UBL Bank', note: 'Bank transfer · account details provided' },
  { key: 'mcb', label: 'MCB Bank', note: 'Bank transfer · account details provided' },
...
export default function Checkout() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      navigate(`/payment/${data.order._id}`, { state: { intent: data.intent, reference: data.order.reference, paymentMethod: method } });
    } catch (err) {
