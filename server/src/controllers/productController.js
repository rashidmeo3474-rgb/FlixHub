...
=======
    ...
    prices: Object.fromEntries([1, 3, 6, 12].map((m) => [m, p.priceFor ? p.priceFor(m) : Math.round(p.monthlyPrice * (m === 3 ? 2.7 : m === 6 ? 5 : m === 12 ? 9 : 1))]))
  }));
};
...
const buildFallbackProducts = () => FALLBACK_PRODUCTS.map((product) => ({
  ...
  inStock: 8,
  prices: Object.fromEntries([1, 3, 6, 12].map((m) => [m, Math.round(product.monthlyPrice * (m === 3 ? 2.7 : m === 6 ? 5 : m === 12 ? 9 : 1))]))
}));
